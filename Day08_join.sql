-- ****** yan yana tabloları gorebiliriz ama yeni bir tablo olustur olmayiz,
-- kalici olarak degistirmez select,
-- alter tablonun inşaatinda değişiklik yapar
-- JOIN : kalici değişiklik yapmaz..farkli tablolari birleştirmek için kullanilir.
-- ortak küme,soladkini ağırlıklı al,sağdakini ağırlıklı al diyerek
-- 4 farkli çeşit tablo konusu göreceğiz JOİN ile.
-- ORACLE'da 4 farkli tablo birleşimi var
-- MySQL'de 3 farkli çeşitte tablo birleşimi olur. 
-- 1) Inner Join 2) Left Join 3) Right Join
-- kimi daha çok ağırlıklı olmasını istersek onu başa yani(left) yazariz..
-- sol tarafi istersek left join, 
-- sağdakini istersek right join,
-- ortasini istersek inner,
-- hepsini istersek 'left unioun right '
--  FULL JOIN=(left join + union all + right join) 
-- Tablodaki tum sonuclari gosterir
-- **** Joinde bağlama işi ON ile yapilir..
...
------------------------------------------------------------------------------*/  
-- ilk tabloyu from'un yanina (ilk yazilan tablo adi soldaki,ikincisi sağdaki)
-- from'dan sonra yazilan SOLDAKİ TABLO OLUR
-- Joind'den sonra yazilan SAĞDAKİ TABLO olur.

-- ayni olanlar diyorsa INNER JOIN, id'lerin eşit oldugu yerde ON
-- sir ve sip bizim uydurduğumuz şirket isimleri
-- ******* sirket id'leri ayni olan yerden bağla ON ile *****
...
/* -----------------------------------------------------------------------------
ORNEK2: sirketler tablosundaki tum sirketleri ve bu sirketlere ait olan siparis_id ve siparis_tarihleri listeleyen bir sorgu yaziniz.
------------------------------------------------------------------------------*/  
...
FROM
    bolumler AS b
        LEFT JOIN
    personel AS p ON p.bolum_id = b.bolum_id
WHERE
